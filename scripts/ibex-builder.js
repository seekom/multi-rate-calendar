// If there is no console, it will create a console function
...
// The datepicker calendar is courtesy of http://jqueryui.com/demos/datepicker/ where there is full documentation
	...
	ibexBuilder.Initialise = function()
	{
		...
		this.htmlHdrColName = '<td width="{ColWidths.Name}">Property</td>';
...
//		var contentBusy = document.getElementById("contentBusy");
//		if(typeof contentBusy != null){
//			contentBusy.style.display = "none";
//		}
		...
		{	
			...
			for (var rateKey = 0; rateKey < property.MinRates.length; rateKey++)
			{
				var minRateDay = property.MinRates[rateKey];
					...
					// Either a "Sold" cell ...
					if (minRateDay.Status == 'Unavailable'){
						propHtml += this.htmlColDaySold;
					}
					...
					else
					{
						...
						var availInfo = 'Room type - ' + minRateDay.RoomName + '<br>' +
									    'Rate type - ' + minRateDay.RateName + '<br>' +
									    'Minimum stay - ' + minRateDay.MinStay + ' night' + (minRateDay.MinStay > 1 ? 's' : '');
				...
				if(hasAvailability){
					...
					// Replace all OddEven occurences
					html += '</tr>' + this.htmlRow.replace(/{OddEven}/g, rowType) + propHtml.replace(/{OddEven}/g, rowType);
				}
