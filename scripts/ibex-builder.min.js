...
{var endDate=new Date(this.DateFromString(stDate));endDate.setDate(endDate.getDate()+minStay);var sep,url,target=this.TargetWindow;if(this.TargetUrl!='')
...
else if(customUrl!=''){sep=customUrl.indexOf('?')<0?'?':'&';url=customUrl+sep+'datein='+stDate+'&dateout='+this.DateToString(endDate);target='_blank';}
...
{hasAvailability=true;var availClass=minRateDay.Status=='Instant Confirmation'?'available':'onreq';var availInfo='Room type - '+minRateDay.RoomName+'<br>'+'Rate type - '+minRateDay.RateName+'<br>'+'Minimum stay - '+minRateDay.MinStay+' night'+(minRateDay.MinStay>1?'s':'');if(this.AvailabilityAlertThreshold>0&&this.AvailabilityAlertMessage!=''&&minRateDay.TotalAvailable<=this.AvailabilityAlertThreshold)
{availInfo=this.AvailabilityAlertMessage+'<br>'+availInfo;}
var roomRate=data.CurrencySymbol+Math.round(parseInt(minRateDay.MinRate));var dayDetails=this.htmlDayDetails.replace("{PropertyId}",property.Id);dayDetails=dayDetails.replace("{DateYmd}",minRateDay.Date).replace("{MinStay}",minRateDay.MinStay);dayDetails=dayDetails.replace("{CustomUrl}",property.CustomUrl);dayDetails=dayDetails.replace("{AvailabilityInfo}",availInfo).replace("{RoomRate}",roomRate);propHtml+=this.htmlColDayAvail.replace("{AvailClass}",availClass).replace("{DayDetails}",dayDetails);}}
if(hasAvailability){var rowType=oddEven[rowNo++%2];html+='</tr>'+this.htmlRow.replace(/{OddEven}/g,rowType)+propHtml.replace(/{OddEven}/g,rowType);}}
